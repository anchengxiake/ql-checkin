...
def wait_with_countdown(delay_seconds, task_name="微软积分签到"):
    """带倒计时的随机延迟等待"""
    if delay_seconds <= 0:
        return
...
class NotificationManager:
    ...
    def _init_notify_client(self):
        """初始化通知客户端"""
        global hadsend, send_func
        if hadsend and send_func:
            class NotifyWrapper:
...
def main():
    ...
    if random_signin:
        ...
        if delay_seconds > 0:
            ...
            wait_with_countdown(delay_seconds, "微软积分签到")
    ...
    print(f"==== 微软积分签到完成 - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} ====")
